"""Creates iceberg objects and extracts data frames from IIP sighting database.
"""
...
class Iceberg:
    ...
    Attributes:
        DIMS_BY_SIZE_CLASS (dict): iceberg dimension ranges according to IIP
        size classes
        RATIOS_BY_SHAPE_CLASS (dict): iceberg shape factors and height to draft
        ratios according to IIP shape classes
    """

    # dictionary values are: [L_min, L_max, W_min, W_max, Hs_min, Hs_max]
    DIMS_BY_SIZE_CLASS = {
                            'GR': [0, 5, 0, 5, 0, 1],
                            'BB': [5, 15, 5, 15, 1, 5],
                            'SM': [15, 60, 15, 60, 5, 15],
    ...
    # dictionary values: [SF, H2D], where SF is shape factor and H2D is the
